...
- Loops: maximum 1 break or continue per for loop
- Fields: always private, exposed via getters only
- Qualifiers: fully qualified names only in import statements
- `var`: never used — always explicit types
- Registries: preferred above all else for extensible concerns
- Annotations: used where meaningful (`@Nonnull`, `@Nullable`, `@Override`, `@FunctionalInterface`, `@ApiStatus.Internal`, `@Layer`)
- No magic: no annotation processors, no reflection scanning, no auto-registration — everything is explicit
