...
import fr.riege.pathfinder.smooth.GradientSmoother;
...
final class PathAssembler {
    ...
    static @NotNull Path assemble(@NotNull List<BlockPos> waypoints, @NotNull PathfinderContext ctx) {
        ...
        List<Vec3> points = samplePoints(waypoints, sampler);
        List<Vec3> smoothed = new GradientSmoother(ctx.collisionLayer(), hitboxHalf).smooth(points);
        List<Segment> segments = buildSegments(waypoints, smoothed);
    ...
    private static @NotNull List<Segment> buildSegments(
            @NotNull List<BlockPos> waypoints, @NotNull List<Vec3> points) {
        List<Segment> segments = new ArrayList<>();
        for (int i = 0; i < waypoints.size() - 1; i++) {
            Vec3 start = points.get(i);
            Vec3 end = points.get(i + 1);
            double length = start.distanceTo(end);
            Node node = new Node(waypoints.get(i), new MovementType(MovementKeys.WALK), 0, 0);
